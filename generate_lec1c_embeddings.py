#!/usr/bin/env python3
"""
Generate embeddings for LEC1C chunks using OpenAI API
"""
...
def main():
    print("=" * 80)
    print("GENERATING LEC1C EMBEDDINGS")
    print("=" * 80)
    ...
    # Get LEC1C book ID
    cursor.execute("SELECT id FROM vedabase_books WHERE code = 'LEC1C'")
    ...
    book_id = result[0]
    print(f"\nFound LEC1C book ID: {book_id}")

    # Get all LEC1C chunks
    print("\nFetching LEC1C chunks from database...")
    cursor.execute("""
        SELECT
            c.id,
            c.verse_id,
    ...
    print(f"  Found {len(chunks)} LEC1C chunks")
    ...
    # Save to file
    output_file = 'lec1c_embeddings_export.json'
