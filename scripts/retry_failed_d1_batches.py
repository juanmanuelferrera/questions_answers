#!/usr/bin/env python3
"""
Retry failed D1 upload batches with ultra-conservative settings
Uses smaller batch size (25) and longer pauses (5s) to work around API issues
"""
...
BATCH_SIZE = 25  # Ultra-conservative batch size
PAUSE_BETWEEN_BATCHES = 5  # Longer pause
...
def upload_batch(batch, batch_num, batch_label):
...
"""
            ...
            sql_file = f"retry_batch_{batch_num}.sql"
            ...
            if result.returncode == 0 and 'success' in result.stdout.lower():
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    print(f"Retry {attempt + 1}/{MAX_RETRIES} (wait {wait_time}s)...", end=" ", flush=True)
            ...
            if attempt < MAX_RETRIES - 1:
                wait_time = 2 ** attempt
                print(f"Timeout, retry {attempt + 1}/{MAX_RETRIES} (wait {wait_time}s)...", end=" ", flush=True)
                time.sleep(wait_time)
                continue
            ...
            if attempt < MAX_RETRIES - 1:
                wait_time = 2 ** attempt
                print(f"Error, retry {attempt + 1}/{MAX_RETRIES} (wait {wait_time}s)...", end=" ", flush=True)
                time.sleep(wait_time)
                continue
            return False, str(e)
...
def main():
    print("=" * 80)
    print("RETRYING FAILED D1 UPLOAD BATCHES")
    print("=" * 80)
    ...
    for i in range(0, missing_count, BATCH_SIZE):
        ...
        # Get chunk ID range for this batch
        batch_ids = [chunk[0] for chunk in batch]
        id_range = f"IDs {min(batch_ids)}-{max(batch_ids)}"

        print(f"[{batch_num}/{num_batches}] Batch {i+1}-{batch_end} ({id_range})...", end=" ", flush=True)

        # Upload batch
        success, error = upload_batch(batch, batch_num, id_range)
    ...
    if failed_batches:
        ...
        print("   You may want to retry these specific batches again")
    else:
    ...
    if final_count >= len(local_ids):
        ...
        print(f"   Total lecture segments in D1: {final_count:,}")
    else:
        print(f"\n⚠️  Still missing: {len(local_ids) - final_count:,} chunks")
        print("   Run this script again to retry failed uploads")
