#!/usr/bin/env python3
"""
Generate embeddings for CC books (Caitanya Caritamrita) chunks
Generates embeddings for the 3,868 CC chunks using OpenAI API
"""
...
def main():
    print("=" * 80)
    print("GENERATING CC BOOK EMBEDDINGS")
    print("=" * 80)
    ...
    # Get CC book IDs
    cursor.execute("SELECT id FROM vedabase_books WHERE code IN ('cc1', 'cc2', 'cc3')")
    cc_book_ids = [row[0] for row in cursor.fetchall()]

    print(f"\nFound CC book IDs: {cc_book_ids}")

    # Get all CC chunks
    print("\nFetching CC chunks from database...")
    cursor.execute("""
        SELECT
            c.id,
            c.verse_id,
    ...
    print(f"  Found {len(chunks)} CC chunks")
    ...
    # Save to file
    output_file = 'cc_embeddings_export.json'
